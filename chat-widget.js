/* =========================================================
   ...
   Встраивается на любую страницу GitHub Pages.
   FAQ-матчинг — полностью на клиенте.
   ========================================================= */
(function () {
  'use strict';
  ...
  // ── FAQ (same as backend) ──
  const FAQ = [
    { q: 'Что такое BIOMA LAB?', a: 'BIOMA LAB — R&D компания, разрабатывающая ветеринарные синбиотики (пробиотики + пребиотики) для сельскохозяйственных и домашних животных.', kw: ['bioma', 'lab', 'компания', 'кто', 'чем', 'занимается', 'это', 'что'] },
    { q: 'Что такое синбиотик?', a: 'Синбиотик — комбинация пробиотика (полезных бактерий) и пребиотика (питательной среды). BIOMA LAB использует S. boulardii + FOS.', kw: ['синбиотик', 'пробиотик', 'пребиотик', 'boulardii', 'fos', 'это', 'что', 'такое'] },
    { q: 'Для каких животных подходит?', a: 'Разработки BIOMA LAB предназначены для сельскохозяйственных животных (КРС, свиньи, птица) и домашних питомцев (собаки, кошки).', kw: ['животные', 'собаки', 'кошки', 'крс', 'свиньи', 'птица', 'поросят', 'телят', 'цыплят'] },
    { q: 'Где купить?', a: 'BIOMA LAB работает B2B. Для коммерческого предложения — напишите нам в Telegram @BiomaLab_bot.', kw: ['купить', 'цена', 'заказать', 'где', 'приобрести'] },
    { q: 'Чем отличаетесь от конкурентов?', a: 'Ключевое отличие — патентованный штамм S. boulardii и научно-обоснованная комбинация с FOS.', kw: ['конкурент', 'отличие', 'уникальный', 'патент', 'штамм'] },
    { q: 'Контакты', a: 'Telegram: @BiomaLab_bot · Сайт: vetbiom.ru', kw: ['контакт', 'связаться', 'написать', 'телефон', 'email', 'почта'] },
    { q: 'Что такое FOS?', a: 'FOS (фруктоолигосахариды) — натуральные пребиотики, питательная среда для полезных бактерий.', kw: ['fos', 'фруктоолигосахарид', 'пребиотик'] },
    { q: 'У кошки/собаки расстройство — поможет?', a: 'Наши синбиотики на основе S. boulardii + FOS поддерживают здоровье кишечника у кошек и собак. Для точной схемы — обратитесь к ветеринару.', kw: ['кот', 'кошк', 'собак', 'срет', 'понос', 'диаре', 'расстройств', 'живот', 'жкт'] },
    { q: 'Дозировки для животных?', a: 'Дозировки зависят от вида, возраста и формы выпуска. Для коммерческого предложения напишите в Telegram @BiomaLab_bot, указав вид и поголовье.', kw: ['дозировк', 'сколько', 'давать', 'грамм', 'мл', 'норм'] },
  ];

  const CHAT_LINK = window.location.hostname === 'maxusoff1.github.io'
    ? 'http://5.35.125.79:8080/chat/'
    : '/chat/';
  ...
  function findFAQ(text) {
    ...
    return bestScore >= 2 ? bestEntry.a : null;
  }
  ...
  panel.innerHTML = `
    ...
    <div id="bioma-widget-msgs">
      <div class="bioma-w-msg bot">
        Здравствуйте! Я консультант BIOMA LAB. Спросите о синбиотиках, здоровье животных или продукции.
  ...
  function ask(text) {
    ...
    setTimeout(() => {
      ...
      if (faq) {
        ...
        const fallback = 'На этот вопрос я отвечу точнее через полную версию чата. '
          + '<br><br><a href="' + CHAT_LINK + '" target="_blank" rel="noopener" '
          + 'style="color:#335341;font-weight:600;text-decoration:underline;">'
          + 'Открыть чат с ИИ →</a>'
          + '<br><br>Либо напишите в <a href="https://t.me/BiomaLab_bot" '
